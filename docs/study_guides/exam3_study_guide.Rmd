...
## silently discards this directory's _output.yml - see docs/_site.yml.
...
# Exam Logistics:
  ...
  * The exam is scheduled for **Friday, November 20, 2026**, and will be taken in class. An in-class review will be held on **Wednesday, November 18, 2026**
...
Know/be able to define the following terms and concepts:
  ...
  * **Margin of error** - the half-width of a confidence interval, i.e. the standard score times the standard error. It measures the uncertainty or precision associated with estimating a population parameter (such as a population mean or proportion) by quantifying the amount by which a sample estimate might plausibly deviate from the true population parameter at the stated confidence level.
...
## **Computing Sample Size** 
  ...
  * Know how to compute the sample size when estimating $\mu$ with $\bar{x}$
	...
	\[ n = \frac{\sigma^2z^2}{m^2} \]
	  ...
	  * $p$ and $\sigma$ must be determined from prior information or studies
...
### **Significance for a single population proportion**
  ...
  2. State the null and alternative hypotheses, where $p_0$ is the hypothesized value of the population proportion

  * **Right-tailed test:** $H_0: p = p_0 \ \ H_A: p > p_0$

  * **Left-tailed test:** $H_0: p = p_0 \ \ H_A: p < p_0$

  * **Two-tailed test:** $H_0: p = p_0 \ \ H_A: p \neq p_0$
  ...
  * **Two-tailed test:** $p$-value = $P(|z| \geq |z_{\text{obs}}| \ \mid \ H_0 \text{true})$
...
### **Significance test for a single population mean**
  ...
  2. State the null and alternative hypotheses, where $\mu_0$ is the hypothesized value of the population mean
  ...
  * **Two-tailed test:** $p$-value = $P(|t| \geq |t_{\text{obs}}| \ \mid \ H_0 \text{true})$
...
**Steps of a sign test:**
  ...
  2. State the null and alternative hypotheses, where $p$ is the probability of a positive sign:

  * **Two-tailed test:** $H_0: p = 0.5$ $H_A: p \neq 0.5$

  * **Right-tailed test:** $H_0: p = 0.5$ $H_A: p > 0.5$

  * **Left-tailed test:** $H_0: p = 0.5$ $H_A: p < 0.5$

  3. The test statistic $S$ is the total number of positive signs, which under the null follows a binomial distribution $S \sim \text{binom}(n, 0.5)$ where $n$ is the total number of signs

  4. The $p$-value is computed from a binomial distribution under the null (the one-sided forms below assume the observed count $s$ falls in the tail named by $H_A$):
...
**Hypothesis Testing:**
  ...
  1. Assumptions: Two populations are independent; random sampling, requires large $n$ in **both** groups, only valid when

\[ n_1\hat{p} \geq 15, \ \ n_1(1-\hat{p}) \geq 15, \ \ n_2\hat{p} \geq 15, \ \ \text{and} \ \ n_2(1-\hat{p}) \geq 15 \]
  ...
  * Two-tailed test $H_A: p_1 - p_2 \neq 0$: $p$-value $= P(|z|\geq |Z_{obs}| \mid H_0) = 2\times\left[1 - P(z<|Z_{obs}|\ \mid H_0)\right]$
...
\[ (\bar{x}_1 - \bar{x}_2) \pm t_{1-\alpha/2} \times \sqrt{\frac{s_1^2}{n_1}+\frac{s_2^2}{n_2}} \]
  ...
  * where $t_{1-\alpha/2}$ has $\min(n_1 - 1, n_2 - 1)$ degrees of freedom
...
**Hypothesis Testing:**
  ...
  * Two-tailed test $H_A: \mu_1 - \mu_2 \neq 0$: $p$-value $= P(|t|\geq |t_{obs}| \mid H_0) = 2\times\left[1 - P(t<|t_{obs}|\ \mid H_0)\right]$
