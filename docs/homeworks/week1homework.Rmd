...
export_tbl(cbind.data.frame(Flip = 1:15, Result = df[[1]]), 'hw1_coin_flips.csv')
...
export_tbl(cbind.data.frame(`Age Group (Years)` = ag, Counts = cts),
           'hw1_age_distribution.csv', names = c('Age Group (Years)', 'Counts'))

kable(age.dist, format = tbl_fmt, col.names = rep('', 7), booktabs = T, row.names = T)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped', latex_options = c('striped', 'HOLD_position'))
...
kable(export_tbl(spam.dist, 'hw1_spam_types.csv',
                 names = c('Type of Spam', 'Percentage')),
      format = tbl_fmt, booktabs = T, row.names = F)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped', latex_options = c('striped', 'HOLD_position'))
...
gpa_raw = c(2.0, 2.1, 2.3, 2.8, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0,3.0, 3.0, 3.0, 3.1, 3.2, 3.3, 3.3, 3.4,  3.4, 3.4,3.4, 3.5, 3.5, 3.5, 3.5, 3.6, 3.6, 3.7, 3.7, 3.8,3.8, 3.8, 3.8, 4.0, 4.0)

# the printed table is a fill-in-the-blank skeleton, so export the underlying
# 35 GPA observations that students actually work from instead
export_tbl(cbind.data.frame(Student = seq_along(gpa_raw),
                            `High School GPA` = gpa_raw,
                            `GPA Rounded to Nearest 0.5` = round_any(gpa_raw, 0.5)),
           'hw1_gpa_values.csv',
           names = c('Student', 'High School GPA', 'GPA Rounded to Nearest 0.5'))

x=round_any(gpa_raw, 0.5)
