...
# the alpha and critical-value columns are blank for students to fill in, so
# export only the confidence levels they are given
export_tbl(cbind.data.frame(`Confidence Level (%)` = 100*(1-alpha)),
           'hw4_confidence_levels_proportion.csv',
...
# blank columns are for students to fill in; export the given values only
export_tbl(cbind.data.frame(`Confidence Level (%)` = 100*(1-alpha),
                            `Sample Size` = n),
           'hw4_confidence_levels_mean.csv',
           names = c('Confidence Level (%)', 'Sample Size'))
...
kable(export_tbl(df, 'hw4_political_party.csv', digits = 2,
                 names = c('Political Party Affiliation', 'Count', 'p-hat')),
...
kable(export_tbl(df, 'hw4_membership_satisfaction.csv', digits = 2,
                 names = c('Response', 'Count', 'p-hat')),
      format = tbl_fmt, digits = 2, row.names = F, booktabs = T, escape = F,
...
kable(export_tbl(df, 'hw4_housing_summary.csv', digits = 2,
                 names = c('Variable', 'Sample Mean', 'Sample Standard Deviation')),
      format = tbl_fmt, digits = 2, row.names = F, booktabs = T, escape = F,
