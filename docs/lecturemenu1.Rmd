...
<span style="font-size:80%">Sources: J. Graunt (1662), *Natural and Political Observations Made upon the Bills of Mortality*. E. Halley (1693), *Phil. Trans. R. Soc.* 17:596-610. G. Achenwall (1749), *Abriss der neuesten Staatswissenschaft*. A. Quetelet (1835), *Sur l'homme et le developpement de ses facultes*. R. A. Fisher (1925), *Statistical Methods for Research Workers*.</span>
...
The following example is data on 20 clients contacted during a bank's direct marketing campaign for a term deposit account. For each client, the client ID, job, education level, age, average account balance, length of the marketing call, and whether the client subscribed are recorded.

```{r, echo=FALSE, message=FALSE, warning=FALSE}

bank.data = read.csv('Data/bank_marketing_example.csv', check.names = FALSE)
bank.data$`Balance ($)` = formatC(bank.data$`Balance ($)`, format = 'f', digits = 2, big.mark = ',')
bank.data$`Call Length (min)` = formatC(bank.data$`Call Length (min)`, format = 'f', digits = 1)

kable(bank.data, format = 'html', row.names = F, booktabs = T)%>%
  kable_classic(full_width = F, html_font = "Cambria")%>%kable_styling(bootstrap_options = 'striped')
```

<span style="font-size:80%">Source: variable structure adapted from Moro, S., Rita, P., and Cortez, P. (2014). *Bank Marketing* [Dataset]. UCI Machine Learning Repository. <https://doi.org/10.24432/C5K306>. The 20 client records shown here are simulated for illustration; the real data set contains 45,211 clients.</span>
...
* Variables are distinguished by the type of information they represent. One basic distinction is between **qualitative** variables and **quantitative** variables. 
  ...
  * **qualitative variables** - also called categorical variables, represent non-numeric qualities or characteristics that can be placed in distinct categories. What are the qualitative variables in the bank marketing data above?
...
* Qualitative variables can be further divided between **nominal** and **ordinal** variables. 
  ...
  * **nominal variables** - non-numeric qualities or characteristics that can be placed in distinct categories that do not have a natural ordering. Examples of variables that cannot be ordered are gender, race, eye color, or political party. In the bank marketing data, which variables would be considered nominal variables?
